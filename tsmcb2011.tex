...
by the robot.
It has to be noted that our approach relies on some strong assumptions but they are
related to the control framework.
...
(no algorithmic singularities~\cite{chiaverini97}), that the set
of active tasks is constant during the motion and the tasks start and finish at the same time.
...
since all tasks are assumed to be completely fulfilled and
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=0.40\linewidth]{img/spotDiff3.ps}
\end{center}
...
body part (see Fig.~\ref{fig:hrp2Markers}).
\begin{figure}[t]
  \centering
  \begin{tabular}{cc}
...
On the robot, a flexibility, due to the mechanical design, is introduced after the ankle joint that
...
However, the task \emph{Head} is not kept as a candidate for any of the two motion (the residue $r$ for the task is never the lowest).
