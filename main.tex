...
flow as a 3D function)~\cite{laptev}. These points are learned from a database, then
matched during the demonstration. In robotics,
the structure of the multi rigid body moving is often supposed as known,
coming from specific limb-motion tracker \cite{condensation} or
from specific marker-based tracking systems \cite{?}. In the following,
we will consider that the motion of the rigid bodies is known.
...
Such as the task function~\cite{samson91} that expresses a control objective in a n-dimensional task space.
...
Under the hypothesis that the motion has been generated by stacking the objectives controls,
...
Projections of the motion in the orthogonal already-detected task spaces
...
of motion. This composition mode, alog with the obvious composition by temporal
sequencing offers a real versatility: subparts of the motion can be used in
very different situations without to redesign them for each case.
...
a $task_i$ should not disturb $task_j$ if $i>j$.
...
In the previous section was recalled the classical widely-used task-function formalism.
...
The observed motion is supposed to have been generated using a sub-set of those tasks.  
It is also assumed that all the tasks involved in the demonstration are compatible
in the sense of the projection $\mbf{P}$:
there is no algorithmic singularities~\cite{chiaverini97} and the set
...
the joint trajectories. We will show in the experimental part how the joint
...
on a curve fitting score: trajectories are projected
in each task space and the projected trajectories are
compared to a theoretical trajectories which represent are characteristic
of the execution of a task.
The observed motion is then projected
onto the null-space of the selected task, cancelling its effect in the observed motion.
...
of the motion is denoted $\mathbf{\dot{q}}^{*}(t)$,
$\mbf{P\dot{q}}(t)$ denotes the successively projections of the reference
motion. Before teh first iteration $\mbf{P\dot{q}}(t)$ is set to the reference motion.
Then each iteration projects it in the selected-task null space
$r_i$ the score of the cost function of the optimization, $activePool$
...
If the observed motion is exactly generated by a SoT, ther resulting trajectory after projection
onto all the active-task spaces will be null. However, in presence of noise like when
acquiring the motion through real sensors,
a residue is systematically obtained, which implies to use a threshold
as stop critera: the loop ends when the residue is below the waited noise of
the acquisition chain.

The next subsection describes the two main functions of the algorithm.
The task fitting is described
...
the observed motion, is achieved by applying a least-square optimization
...
The first experiment validates the projection of the motion. Fig.~\ref{fig:snapshotXpqdot} 
and Fig.~\ref{fig:xp3Pqdot} illustrate the effect of the projections.
The second experiment compiles a set of experiments that validate the task recognition algorithm.
Fig.~\ref{fig:XP2RFit} and Fig.~\ref{fig:XP2RLFit} shows how the fitting behave. 
...
The description of the tasks are detailed below, while Fig.~\ref{fig:introExample:spotDiff},
...
\centering
  ...
  \caption{Evolution of the commands relevant to the \emph{right grab} (in solid lines) and 
  \emph{left grab} (in dashed lines) tasks after
...
real HRP-2 using a motion capture system. (Fig.~\ref{fig:camera},~\ref{fig:hrp2Markers}).
The task recognition algorithm is applied to two set of similar looking motions.
