...
The learned primitives are often sequential : a motion is segmented according to the time axis.
...
In the vision community, the problem is generally looked from the
...
The structure of the multi rigid body moving is often supposed as known
and is used to estimate the pose. For example the estimation
...
Under the hypothesis that the motion has been generated by stacking a set of controlers,
...
as well as its execution since the subspace is generally closely linked with the sensors of
the robot. Based on the redundancy of the system, the
...
For example, the observed feature can be the 3D position $\mbf{p}$ of one of
the robot end-effector, to be brought to a chosen position $\mbf{p^*}$.
...
we propose to use the tasks as a set of descriptor to recognize a demonstrated motion,
...
It is assumed that the model behavior ($\mbf{e}$, $\mbf{\dot{e}^*}$, $\mbf{J}$) of every
...
in the sense of the projection $\mbf{P}$
(no algorithmic singularities~\cite{chiaverini97}) and the set
of active tasks is constant during the motion.
Each tasks are completely fulfilled. Finally, the observed motion is given through
the joint trajectories. The joint angles trajectories can be recovered using a motion
capture system as shown in the remainder of this paper.
trajectories can be recovered using a motion capture system.

\subsection{Overview}
\label{sec:alg1:selec}
The input of the algorithm is the joint trajectories the task pool. The algorithm is iterative: at each iteration, the task that seems the
most relevant is selected. The selection of a task relies
on a curve fitting score, obtained by projecting the joint angles trajectory in each task space.
...
The algorithm stops when the original motion iteratively projected
is totally canceled.

The algorithm is showed in Alg.~\ref{alg:taskSelection}.
The joint-angles velocity trajectory
of the observed motion is denoted $\mathbf{\dot{q}}^{*}(t)$.
$\mbf{P\dot{q}}(t)$ denotes the successively projections of the reference
...
If the observed motion is exactly generated by a SoT, the resulting trajectory $\mathbf{P\dot{q}}$ after projection
onto all the active-task spaces is null. However, in presence of noise (like when
acquiring the motion through real sensors)
a residue is systematically obtained, which implies to use a threshold
as stop criteria: the loop ends when the residue is below the waited noise of
...
The procedure $\mathrm{projection}(i, \mathbf{\dot{q}}(t))$ compute the projector onto
...
In this section, the projection used to decouple the tasks involved in the motion is detailed.
%\subsubsection{General case}

In order to cancel a task $i$, the joint angles trajectory is projected onto the nullspace
of that task by multiplying it by the projector onto the nullspace of all tasks to cancel.
...
In the following example, a motion composed of two arbitrary tasks is analyzed, and 
the effect of the projections is detailed. 
The control law for a motion composed of two tasks is given by:
\begin{equation}
  \mbf{\dot{q}} = \mbf{J_1^+}\mbf{\dot{e_1^*}} + (\mbf{J_2}\mbf{P_1})^+ (\mbf{\dot{e_2^*}} - \mbf{J_2}\mbf{J_1^+}\mbf{\dot{e}_1^*})
  \label{eq:controlLaw2Tasks}
\end{equation}

Multiplying~(\ref{eq:controlLaw2Tasks}) by $\mbf{P_1}$ cancels the motion in the Task 1 space:
\begin{equation}
  \mbf{P_1} \mbf{\dot{q}} = \mbf{P_1} \mbf{J_1^+}\mbf{\dot{e_1^*}} + \mbf{P_1} (\mbf{J_2}\mbf{P_1})^+ (\mbf{\dot{e_2^*}} - \mbf{J_2}\mbf{J_1^+}\mbf{\dot{e}_1^*})  
  \label{eq:cancel1_2:1}
\end{equation}
The first term is null by definition of $\mbf{P_1}$.
In order to observe the motion which lies in the Task 2 space after the nullification
of the Task 1 component, the result of the projection is
multiplied by the task Jacobian $\mbf{J_2}$, this operation leads to :
\begin{equation}
  \mbf{J_2} \mbf{P_1} \mbf{\dot{q}} = \mbf{\dot{e_2}^*} +
  \mbf{J_2}  \mbf{J_1^+} \mbf{\dot{e_1^*}}  
  \label{eq:J2P1q}
\end{equation}
...
the observed motion, is achieved by applying a non linear least-square optimization
between the actual observed motion and the reference behavior of a task over the parameters
of the reference behavior :
\begin{equation}
  \mbf{x}^* = \underset{\mbf{x}}\argmin \frac{\int \Vert \mbf{e}^*(t) - \mbf{e}_\mbf{x}(t) \Vert^2 \mathrm{dt}}{\int \Vert \mbf{e}^*(t) \Vert^2 \mathrm{dt}}
\label{optimProblem}
\end{equation}

\noindent where $\mbf{e}^*(t)$ is the observed trajectory, $\mbf{e}_\mbf{x}(t)$ is the trajectory
generated by the model using the parameters $\mbf{x}$. 
In some cases the trajectory $\mbf{e}^*$ can be observed directly. For example, if the task space 
is the 3D position of the hand, a direct observation is possible.
However, in the general case, a direct observation is not possible. For example,
the center of mass of the robot is difficult to observe directly. 

So $\mbf{e}^*$ can be obtained with the kinematic model and 
the joint-angle trajectory of the robot: $\mbf{J_i}\mbf{\dot{q}^*}$
But due to the coupling, it is better to introduce in $\mbf{e}^*$ the coupled 
component seen in equation (\ref{eq:J2P1q}): $\mbf{J_i}  \mbf{J_A^+} \mbf{\dot{e_A^*}}$
where $A$ is the set of tasks that have already been detected and $i$ is the task candidate.

In practise, the observation is sampled and the integral is indeed a sum.
...
The proof is demonstrated on a motion composed of two tasks, but can be easily
generalized to $n$ tasks.
On one hand, the Task 1 then Task 2 is nullified, on the other hand, Task 2 then Task 1 is nullified.
                                            
\subsubsection{Removing Task 1, then Task 2}
Starting from the original control law~(equation~(\ref{eq:controlLaw2Tasks})),
the nullification of the Task 1 leads to equation~(\ref{eq:cancel1_2:1}).
...
(obtained in equation~(\ref{eq:cancel1_2:1})).
\begin{equation}
...
\medskip
\subsubsection{Removing Task 2, then Task 1} 
The same result is obtained if the task $2$ is cancelled first.
Removing the Task 2 leads to:
\begin{equation}
  \mbf{P_2}\mbf{\dot{q}} = \mbf{P_2} \mbf{J_1^+}\mbf{\dot{e}_1^*} + \mbf{P_2} (\mbf{J_2}\mbf{P_1})^+ (\mbf{\dot{e_2^*}} - \mbf{J_2}\mbf{J_1^+}\mbf{\dot{e}_1^*})
  \label{eq:P2q}
\end{equation}
where the second term is null. Then removing Task 1 leads to:
\begin{equation}
  \mbf{P_{1, 2}} \mbf{P_2}\mbf{\dot{q}} = \mbf{P_{1, 2}} \mbf{P_2} \mbf{J_1^+}\mbf{\dot{e}_1^*}
  \label{eq:P1P2q}
\end{equation}
which is null because $\mbf{P_{1, 2}} = \mbf{P_{2, 1}} \subset \mbf{P_1}$ and $\mbf{P_1}\mbf{J_1^+} = 0$.

This proves that detecting the Task 1 after Task 2 is equivalent to detect the Task 2 after the Task 1.

\section{Results in simulation}
\label{sec:simu}
This section details a series of experimentation in simulation related to this work.
Simulation allows us ti emphasize the nominal behavior of the algorithm without 
any sensor noise.
The first experiment simply validates the projection of the motion~\ref{sec:prelimValid}. 
Fig.~\ref{fig:snapshotXpqdot} and Fig.~\ref{fig:xp3Pqdot} illustrate the effect of the projections.
The second part compiles a set of experiments that validate the task recognition algorithm~\ref{sec:distinc}.
...
The description of the tasks are detailed below.
...
\begin{figure}[t]
  ...
  \end{center}
  \caption{Left: The final position of the motion 2.a; Right: The final position of the motion 2.b.}
  \label{fig:spotDiff1}
\end{figure}
...
\begin{itemize}
  ...
  \item \emph{Twofeet}: constraint the both feet to stay flat with regard to each other on the ground (6 DOF)
...
of the robot reaches a point defined in the Cartesian space (3 DOF)
  \item \emph{Screw} : is similar to the \emph{grab} task, but the desired position (6 DOF)
has to be reached with a defined orientation.
  \item \emph{Head} : is reaching task constraining in position and orientation the head of the
    robot. (6 DOF)
...
which select the tasks that are fitted the most closely by the optimization.
...
are sufficient to disambiguate similar-looking motion. Three ambiguous pair of movements
...
In the first case, the motion of the left hand is due of the \emph{com} and \emph{right hand} task, 
...
However, in the proper task spaces, those motions appears clearly as different.
...
is $\epsilon = 0.1$.
Finally Fig.~\ref{fig:RbeforeAfterProj} shows how the norm of the joint-angle velocities relevant to the tasks
...
to another task. Therefore, the task selection algorithm will still look
for the task that has controlled the left arm.
After the detection of the two (for the motion 2.a) and three (for motion 2.b) main tasks,
the norm of the last joint angles velocities are not null, because 
...
\centering
  ...
  of the task \emph{com}. In the motion 2.a, a great part of the motion of the left arm is due to 
  the \emph{com} task: removing the \emph{com} task will cancel almost all motion in the left arm.
  In the motion 2.b, the motion of the left arm is not only involved by the \emph{com} task
...
generated by the task \emph{grab}. The \emph{grab} taskis then first selected for motion 3.a.
...
While the robots leans, the left hand is dragged by the chest as an involuntary side effect.
The position and orientation of that hand is recorded as a desired state of the
hand for the first \emph{motion 4.b} task.
In \emph{motion 4.b}, the gaze is thus uncontrolled. The motion of the head is,
...
As previously, the correct tasks are detected for each movements. The residue after projections on all detected
task is very small, proving that everything was properly detected and subtracted.

In conclusion, we have shown experimentally in this section that, without noise, the detection algorithm
performs ideally, ie detected exactly all the active tasks, without any  false detection,
and issue on a neglectable residue of projected motion. The following section will consider
the realistic case of noisy signals acquired by real sensors.
...
in realistic situation in presence of noise. However this time, the reference motion 
is demonstrated by the real HRP-2 and observed 
using a motion capture system. (Fig.~\ref{fig:camera},~\ref{fig:hrp2Markers}).
As previously, the task recognition algorithm is applied to two set of similar looking motions.
In a first time, we will briefly explain how the motion capture system is used to record the joint-angle 
trajectory. Then two pairs of motions are observed and detected.
Fig.~\ref{fig:exp1:headFit} and Fig.~\ref{fig:exp1:taskLhand} show how the fitting performs.
Fig.~\ref{fig:exp1:Evolution2L} and Fig.~\ref{fig:exp1:Evolution2R} show how the projected trajectories
evolves, Fig.~\ref{fig:exp1:PqdotNormsRL} and Fig.~\ref{fig:exp6:PqdotNorms5} show the evolution
of the norm of the motion after successive projections. Fig.~\ref{fig:motion5} shows the final
positions of motions applied to the robot.

\subsection{Experimental setup}
A motion is executed with the \emph{SoT} on the Robot HRP-2 equiped with markers on each
...
The result of this process is the 6D trajectory of all the bodies in space, without any joint constraints.
\begin{figure}[t]
  \centering
  \begin{tabular}{cc}
...
The joint angle trajectory is computed as classically done by optimizing the distance
...
transformation matrix $\tensor[^{W}]{\mathbf{\hat{R}}}{_{q_i}}(t)$ using the joint limits
...
\begin{equation}
  \tensor[^{W}]{\mathbf{\hat{R}}}{_{q_i}}(t) = \tensor[^{W}]{\mathbf{R}}{_{W_{mocap}}} \times \tensor[^{W_{mocap}}]{\mathbf{\hat{R}}}{_{M_i}}(t) \times \tensor[^{M_i}]{\mathbf{R}}{_{q_i}}    
  \label{mocapMatrix}
\end{equation}
where $\tensor[^{W}]{\mathbf{R}}{_{W_{mocap}}}$ and $\tensor[^{M_i}]{\mathbf{R}}{_{q_i}}$ are constant
displacement due to the differences between the arbitrary skeleton model of the motion 
capture and our kinematic model of the robot,
computed in a calibration step.
$\tensor[^{W_{mocap}}]{\mathbf{\hat{R}}}{_{M_i}}$
is the measured transformation matrix from the origin of the motion capture world
to the virtual body $i$. 

The obtained joint angles trajectory is then used for the reverse engineering analysis.\\
...
neither in the control algorithm nor the detection method. So
...
hand the balancing \emph{com} task. The second motion is a reaching of both hands, built to be 
ambiguous with regard to the first motion~(see Table~\ref{tab:motion}).
...
The order of the task extraction is :
...
\begin{figure*}[t]
  ...
  For both motion, $r$ is high: the task is not selected.}
  \label{fig:exp1:headFit}
\end{figure*}
Although no \emph{head} task are involved in the motions, the head is not 
fixed in space in both motions due to the motion of the neck for balancing, and the chest and the legs
joints for all the tasks.
...
However, the task \emph{head} is not kept as a candidate for any of the two motion (residue $r$ is high).
...
spaces after the successive projection onto the automatically-selected nullspaces
...
Despite of the noise, the active tasks are detected. The two motions are
properly disambiguated. Finally, all the motion is properly discovered, as proved by the 
very small residue.
\begin{table}[t]
  \centering
  \begin{tabular}{|c|c|c|c|}
...
at each algorithm iteration. The quantity of motion finally remaining at the end of 
the algorithm loop is clearly only due to noise.
\begin{figure*}[t]
  \centering
...
of what is a task and how it behaves (exponential decrease). 
...
any control laws can be used to generate the motion can be similarly
detected to characterize an observed movement.
...
However, it is directly possible to handle other task models, like for example minimum jerk,
