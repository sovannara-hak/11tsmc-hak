...
can be computed to perform the recognition.
%%%%%%%%%%% whaaat?
...
A task function~\cite{samson91} expresses a control objective as a regulation to zero function.
...
\section{Reverse engineering the stack of tasks} 
\label{sec:detect}
...
The observed motion is generated by a SoT.  The idea is to identify which of 
the possible controllers were in the stack of tasks. 
...
\end{algorithmic}
\end{algorithm}
in the following, the joint angles velocities trajectory
...
In a first time, we prove the validity of the approach for both detection 
and projection without any noise, by performing the motion in simulation.
In a second time, we will experimentally demonstrate the validity of the approach
in realistic situation in presence of noise by observing the reference motion on the
real HRP-2 using a motion capture system.
...
freeflyer). All reference motions start from the half-sitting pose showed in Fig.~\ref{fig:halfSit}.
