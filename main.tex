...
Fig.~\ref{fig:RbeforeAfterProj} shows how the norm of the commands (ie. joint angles velocities) relevant to the tasks
...
the \emph{right grab} task will decrease its associated theoritical command while leaving the
\emph{left grab} one unchanged. The original theoretical commands of the \emph{right grab}
and \emph{left grab} are ploted respectively in dark red and dark blue and
the theoretical commands of the \emph{right grab} and \emph{left grab} after projection
into the nullspace of the \emph{right grab} task is ploted in medium red and medium blue
in Fig.~\ref{fig:RbeforeAfterProj:2a}. The theoretical commad associated to the 
\emph{left grab} task is decreased after the projection of the motion in
the nullspace of the \emph{com} task (the norm of the commands are ploted in light red and light blue).
...
and \emph{com} task the command associated to the \emph{left grab} task
...
It has to be noted that the norm of the last commands are not null, because 
...
Although no \emph{head} task are involved in the motions, we can 
notice that in the \emph{motion 2.b}, the candidate \emph{head} task involves less motion
...
the motion of the left hand is cancelled after removing the \emph{com} task.
For the \emph{motion 2.b} the motion of the left hand is cancelled after
