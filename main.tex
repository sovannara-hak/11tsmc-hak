...
the controllers.
The approach is successfully applied in simulation and on a real robot
...
are expressed by some score functions inspired by neuroscience: the proposed criteria are
...
seek the known behaviors in each task spaces. 
Moreover, while all the approaches presented above can only deal with non-simultaneous task to recognize,
we rely on the redundancy-control principles to recognize sets of simultaneous tasks.
...
executed in the task space given a signal $s$ and its desired value $s^*$. 
...
between a current position $\mbf{p_3}$ and its desired value
\begin{eqnarray*}
...
name \emph{Stack of Tasks} (SoT). The next section details how those properties are
...
The observed motion is then generated using a sub-set of those tasks.  
...
In order to iteratively recognize
simultaneous tasks, the observed motion is projected
onto the null-space of the task that involves the best fitting, cancelling it in the motion.
Then the other tasks can iteratively be recognized using the same mechanism with the projected motion.
