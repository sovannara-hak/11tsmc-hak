...
in the sense of the projection $\mbf{P}$ defined (\ref{eq:ltsq})
...
$\mbf{e}^*$ is then obtained by the kinematic model and 
joint trajectories of the robot: $\mbf{J_i}\mbf{\dot{q}^*}$
...
\begin{equation}
  \mbf{e^*}(t) = \mbf{J_i}\mbf{P\dot{q}}(t) + \mbf{J_i}\mbf{J_A^+}\mbf{\dot{e}_A}
...
%
In practice, the observation is sampled and the integral is indeed a sum.
...
On one hand, the Task 1 then Task 2 is nullified, on the other, Task 2 then Task 1 is nullified.
...
At iteration 1, the observation in task space 1 is directly $\mbf{J_1\dot{q}^*} = \mbf{\dot{e}_1^*}$.
In task space 2, it is:
\begin{align*}
...
The motion at the second iteration is null when projected in task space 1. In task space 2 it is
\begin{equation}
  \mbf{e_2}^* = \mbf{J_2(P_1 \dot{q})} + \mbf{J_2} \mbf{J_1^+} \mbf{\dot{e}_1^*} = \mbf{\dot{e}_2}
  \label{eq:proof:J2P1q}
\end{equation}
The same observation is obtained in task space 2 independently from the projection in the 
nullspace of task 1.
\subsubsection{Removing Task 2, then Task 1} 
If projecting the original motion~(\ref{eq:controlLaw2Tasks}) in the nullspace of task 2 at the 
end of the first iteration, the resulting motion is:
\begin{equation}
  \mbf{P_2}\mbf{\dot{q}^*} = \mbf{P_2}\mbf{J_1}^+\mbf{\dot{e}_1^*} + 
  \mbf{P_2}(\mbf{J_2}\mbf{P_1})^+ (\mbf{\dot{e}_2^*} - \mbf{J_2}\mbf{J_1^+}\mbf{e_1^*})
  \label{eq:proof:P2q}
\end{equation}
It can be shown 
%(appendix 1) 
...
\begin{equation}
  \mbf{e}^* = \mbf{J_1}\mbf{P_2\dot{q}^*}(t) + \mbf{J_1}\mbf{J_2^+}\mbf{\dot{e}_2^*}  
  \label{eq:J1P2q}
\end{equation}
Similarly to previous section, the control law after projection in the nullspace of task 1 at the second iteration is null.
...
This time the extracted tasks are : \emph{Right Grab},  \emph{Com}, \emph{Twofeet} and \emph{Left Grab}.
